...
module('Acceptance | Sign up', function(hooks) {
  ...
  test('Sign up client-side errors', async function(assert) {
    ...
    assert.dom('[data-test-rr=email-error]').hasText('Email should be a valid email', 
    'Email error is displayed');

    await fillIn('#password', 'crooked');
    await triggerEvent('#password', 'blur');

    assert.dom('[data-test-rr=password-error]').hasText('Password should be at least 8 characters',
     'Password error is displayed');

    await fillIn('#email', 'dave@tcv.com');

    assert.dom('[data-test-rr=email-error]').hasText('', 
    'Email error is no longer displayed');

    await fillIn('#password', 'ThemCr00ked!');
    
    assert.dom('[data-test-rr=password-error]').hasText(''
    , 'Password error is no longer displayed');
  });
